* 首先通过父组件的render方法调用createElement(这里的_c)构建父vnode，可能包含多个children(例如:这里_v生成createTextVNode文本vnode、_c生成组件vnode)  === _v、_s等是在 renderMixin中调用installRenderHelpers在Vue原型链上注入的； vm._c是在initRender时注入的
